...
class GenerateTestCoursesCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $courseConnection = GeneralUtility::makeInstance(ConnectionPool::class)->getConnectionForTable('tx_spark_course');
        $syllabusConnection = GeneralUtility::makeInstance(ConnectionPool::class)->getConnectionForTable('tx_spark_course_syllabus');
        ...
        for ($i = 0; $i < $count; $i++) {
            ...
            $courseConnection->insert('tx_spark_course', $course);
            $courseUid = (int)$courseConnection->lastInsertId('tx_spark_course');
            ...
            for ($s = 0; $s < $numSyllabi; $s++) {
                ...
                $syllabusConnection->insert('tx_spark_course_syllabus', $syllabus);
                $syllabusUid = (int)$syllabusConnection->lastInsertId('tx_spark_course_syllabus');
            ...
            $courseConnection->update('tx_spark_course', ['syllabi' => $numSyllabi], ['uid' => $courseUid]);
    ...
    private function loadLookupData(): array
    {
        ...
        $categories = $connectionPool->getConnectionForTable('tx_spark_course_category')
            ->select(['uid'], 'tx_spark_course_category', ['deleted' => 0])
            ->fetchAllAssociative();
        
        $cycles = $connectionPool->getConnectionForTable('tx_spark_study_cycle')
            ->select(['uid'], 'tx_spark_study_cycle', ['deleted' => 0])
            ->fetchAllAssociative();
        
        $statuses = $connectionPool->getConnectionForTable('tx_spark_course_status')
            ->select(['uid'], 'tx_spark_course_status', ['deleted' => 0])
            ->fetchAllAssociative();
        
        $methods = $connectionPool->getConnectionForTable('tx_spark_teaching_method')
            ->select(['uid'], 'tx_spark_teaching_method', ['deleted' => 0])
            ->fetchAllAssociative();
        
        $languages = $connectionPool->getConnectionForTable('tx_spark_language')
            ->select(['uid'], 'tx_spark_language', ['deleted' => 0])
            ->fetchAllAssociative();
        
        $sdgs = $connectionPool->getConnectionForTable('tx_spark_sdg')
            ->select(['uid'], 'tx_spark_sdg', ['deleted' => 0])
            ->fetchAllAssociative();
        
        $fields = $connectionPool->getConnectionForTable('tx_spark_scientific_field')
    ...
    private function addSdgRelations(int $syllabusUid, array $sdgUids, $faker): void
    {
        ...
        $connection = GeneralUtility::makeInstance(ConnectionPool::class)
            ->getConnectionForTable('tx_spark_coursesyllabus_sdg_mm');
        ...
        foreach ($selectedSdgs as $sorting => $sdgUid) {
            $connection->insert('tx_spark_coursesyllabus_sdg_mm', [
        ...
        $syllabusConnection = GeneralUtility::makeInstance(ConnectionPool::class)
            ->getConnectionForTable('tx_spark_course_syllabus');
        $syllabusConnection->update(
            'tx_spark_course_syllabus',
    ...
    private function addTeachingMethodRelations(int $syllabusUid, array $methodUids, $faker): void
    {
        ...
        $connection = GeneralUtility::makeInstance(ConnectionPool::class)
            ->getConnectionForTable('tx_spark_coursesyllabus_teachingmethod_mm');
        ...
        foreach ($selectedMethods as $sorting => $methodUid) {
            $connection->insert('tx_spark_coursesyllabus_teachingmethod_mm', [
                'uid_local' => $syllabusUid,
