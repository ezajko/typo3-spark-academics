...
class GenerateTestProjectsCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $connection = GeneralUtility::makeInstance(ConnectionPool::class)->getConnectionForTable('tx_spark_project');
        
        $io->progressStart($count);
        
        for ($i = 0; $i < $count; $i++) {
            $project = $this->generateProject($faker, $lookups, $pid);
            $connection->insert('tx_spark_project', $project);
            $projectUid = (int)$connection->lastInsertId('tx_spark_project');
    ...
    private function loadLookupData(): array
    {
        ...
        $statuses = $connectionPool->getConnectionForTable('tx_spark_project_status')
            ->select(['uid'], 'tx_spark_project_status', ['deleted' => 0])
            ->fetchAllAssociative();
        
        $types = $connectionPool->getConnectionForTable('tx_spark_project_type')
            ->select(['uid'], 'tx_spark_project_type', ['deleted' => 0])
            ->fetchAllAssociative();
        
        $programs = $connectionPool->getConnectionForTable('tx_spark_funding_program')
            ->select(['uid'], 'tx_spark_funding_program', ['deleted' => 0])
            ->fetchAllAssociative();
        
        $fields = $connectionPool->getConnectionForTable('tx_spark_scientific_field')
            ->select(['uid'], 'tx_spark_scientific_field', ['deleted' => 0, 'level' => 2])
            ->fetchAllAssociative();
    ...
    private function addScientificFieldRelations(int $projectUid, array $fieldUids, $faker): void
    {
        ...
        $connection = GeneralUtility::makeInstance(ConnectionPool::class)
            ->getConnectionForTable('tx_spark_project_scientific_field_mm');
        ...
        foreach ($selectedFields as $sorting => $fieldUid) {
            $connection->insert('tx_spark_project_scientific_field_mm', [
        ...
        $projectConnection = GeneralUtility::makeInstance(ConnectionPool::class)
            ->getConnectionForTable('tx_spark_project');
