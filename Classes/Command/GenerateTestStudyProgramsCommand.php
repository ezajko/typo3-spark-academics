...
class GenerateTestStudyProgramsCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $programConn = $connPool->getConnectionForTable('tx_academics_domain_model_study_program');
        $curriculumConn = $connPool->getConnectionForTable('tx_academics_domain_model_curriculum');
        $semesterConn = $connPool->getConnectionForTable('tx_academics_domain_model_curriculum_semester');
        $groupConn = $connPool->getConnectionForTable('tx_academics_domain_model_course_group');
        ...
        for ($i = 0; $i < $count; $i++) {
            ...
            $programConn->insert('tx_academics_domain_model_study_program', $program);
            $programUid = (int)$programConn->lastInsertId('tx_academics_domain_model_study_program');
            ...
            $curriculumConn->insert('tx_academics_domain_model_curriculum', $curriculum);
            $curriculumUid = (int)$curriculumConn->lastInsertId('tx_academics_domain_model_curriculum');
            ...
            for ($sem = 1; $sem <= 2; $sem++) {
                ...
                $semesterConn->insert('tx_academics_domain_model_curriculum_semester', $semester);
                $semesterUid = (int)$semesterConn->lastInsertId('tx_academics_domain_model_curriculum_semester');
                ...
                $groupConn->insert('tx_academics_domain_model_course_group', $groupM);
                $groupMUid = (int)$groupConn->lastInsertId('tx_academics_domain_model_course_group');
                ...
                $groupConn->insert('tx_academics_domain_model_course_group', $groupE);
                $groupEUid = (int)$groupConn->lastInsertId('tx_academics_domain_model_course_group');
    ...
    private function loadLookupData(): array
    {
        ...
        return [
            'organizations' => $fetchUids('tx_academics_domain_model_organization'),
            'cycles' => $fetchUids('tx_academics_domain_model_study_cycle'),
            'fields' => $fetchUids('tx_academics_domain_model_scientific_field'),
            'languages' => $fetchUids('tx_academics_domain_model_language'),
            'study_types' => $fetchUids('tx_academics_domain_model_study_type'),
            'modes_of_study' => $fetchUids('tx_academics_domain_model_mode_of_study'),
            'courses' => $fetchUids('tx_academics_domain_model_course'),
        ];
    }
    ...
    private function addRelations(int $localUid, string $type, array $foreignUids, int $min, int $max, $faker): void
    {
        ...
        $tableName = 'tx_academics_domain_model_studyprogram_' . $type . '_mm';
    ...
    private function addCourseRelations(int $groupUid, array $courseUids, int $min, int $max, $faker): void
    {
        ...
        $connection = GeneralUtility::makeInstance(ConnectionPool::class)->getConnectionForTable('tx_academics_domain_model_coursegroup_course_mm');
        
        $count = $faker->numberBetween($min, $max);
        $selected = $faker->randomElements($courseUids, min($count, count($courseUids)));

        foreach ($selected as $sorting => $foreignUid) {
            $connection->insert('tx_academics_domain_model_coursegroup_course_mm', [
                'uid_local' => $groupUid,
                'uid_foreign' => $foreignUid,
                'sorting' => $sorting,
                'sorting_foreign' => $sorting,
