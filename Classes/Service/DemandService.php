...
class DemandService
{
    ...
    protected function applyStudyProgramFilters(Demand $demand, array $settings): void
    {
        ...
        $typeUid = (int)($settings['filter']['studyType'] ?? $settings['filter']['study_type'] ?? $settings['filter.study_type'] ?? 0);
        $modeUid = (int)($settings['filter']['modeOfStudy'] ?? $settings['filter']['mode_of_study'] ?? $settings['filter.mode_of_study'] ?? 0);
        $languageUid = (int)($settings['filter']['language'] ?? $settings['filter.language'] ?? 0);
    ...
    {
        $departmentUid = (int)($settings['filter']['department'] ?? $settings['filter.department'] ?? 0);
        $chairUid = (int)($settings['filter']['chair'] ?? $settings['filter.chair'] ?? 0);
        $cycleUid = (int)($settings['filter']['studyCycle'] ?? $settings['filter']['study_cycle'] ?? $settings['filter.study_cycle'] ?? 0);
        $categoryUid = (int)($settings['filter']['courseCategory'] ?? $settings['filter']['course_category'] ?? $settings['filter.course_category'] ?? 0);
        $fieldUid = (int)($settings['filter']['scientificField'] ?? $settings['filter']['scientific_field'] ?? $settings['filter.scientific_field'] ?? 0);
    ...
    {
        $primaryDepartmentUid = (int)($settings['filter']['primaryDepartment'] ?? $settings['filter']['primary_department'] ?? $settings['filter.primary_department'] ?? 0);
        $departmentUid = (int)($settings['filter']['department'] ?? $settings['filter.department'] ?? 0);
        $academicRankUid = (int)($settings['filter']['academicRank'] ?? $settings['filter']['academic_rank'] ?? $settings['filter.academic_rank'] ?? 0);
        $academicTitleUid = (int)($settings['filter']['academicTitle'] ?? $settings['filter']['academic_title'] ?? $settings['filter.academic_title'] ?? 0);
        $groupUid = (int)($settings['filter']['researchGroup'] ?? $settings['filter']['research_group'] ?? $settings['filter.research_group'] ?? 0);
        $labUid = (int)($settings['filter']['researchLab'] ?? $settings['filter']['research_lab'] ?? $settings['filter.research_lab'] ?? 0);
        $chairUid = (int)($settings['filter']['chair'] ?? $settings['filter.chair'] ?? 0);
